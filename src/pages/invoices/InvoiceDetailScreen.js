...
function InvoiceDetailScreen(props) {
  ...
  const toggleViewMode = useCallback(() => {
    if (invoiceForm.statusIndex !== "1" && isViewMode) {
      toast.warn("Sólo puedes editar en modo Borrador", {
  ...
  const openSettingModal = useCallback(() => {
    if (invoiceForm.statusIndex !== "1" && isViewMode) {
      toast.warn("Sólo puedes cambiar la configuración en el modo Borrador", {
        position: "bottom-center",
        autoClose: 3000,
      });
      return;
  ...
  const handlerProductValue = useCallback(
    (event, keyName, productID) => {
      ...
      //Si Clave Precio o Cantidad debe ser sólo número
      if (keyName === "quantity") {
        if (!`${value}`.match(/^\d+$/)) {
          return;
        }
  ...
  const addPercentageTax = useCallback(() => {
    ...
    if (isSomeTaxes) {
      toast.error("¡Ya tiene el porcentaje de impuestos!", {
        position: "bottom-center",
        autoClose: 2000,
      });
      return;
  ...
  const saveAs = useCallback(
    (status) => {
      setStatusData({
        statusIndex: status === "Borrador" ? "1" : status === "Pendiente" ? "2" : "3",
        statusName: status,
      });
  ...
  useEffect(() => {
    if (initLoading === false) {
      if (params.id === "new" && invoiceForm === null) {
        // Si es nuevo ignoro los datos de la empresa,
        // Cada vez que establecemos los datos actuales de la empresa
        setInvoiceForm({
          ...invoiceNewForm,
        ...
        // sino factura existente,
        // Estableceremos los datos de la empresa
        // Pero si es Borrador, estableceremos los datos de Empresa Actual
        // Porque todavía es Estado Borrador establecemos los Datos actuales de la empresa
        const getInvoiceDetail = allInvoiceDetails.find(
          (inv) => inv.id === params.id
        );

        // Si no se encuentra redirigir hacia atrás
        // navegar("/");
        if (!getInvoiceDetail) {
          navigate("/invoices");
          return;
    ...
    if (isConfirm) {
      ...
      if (isNew) {
        ...
        setInvoiceForm({
          ...
          products: [
            {
            //AQUI ES PARA CAMBIAR LAS VARIABLES PREDERTEMEINADAS DE LA FACTURA UNA FACTURA NUEVA
              amount: 1200,
              id: nanoid(),
    ...
    <div>
      ...
      {invoiceForm && (
        ...
        >
          ...
          >
            ...
            <div className="flex-1">
              <div className="flex flex-row justify-between items-center mb-1">
                <div className="font-title flex-1"> FACTURA # </div>
              ...
              <div className="flex flex-row justify-between items-center mb-1">
                <div className="font-title flex-1"> Fecha de creación </div>
                <div className="font-title flex-1 text-right">
                  <DatePicker
              ...
              <div className="flex flex-row justify-between items-center mb-1">
                <div className="font-title flex-1"> Fecha de vencimiento </div>
                <div className="font-title flex-1 text-right">
                  <DatePicker
          ...
          {/* Información de facturación del cliente Finalizado*/}
          ...
          <div className="py-2 px-4">
            ...
            >
              ...
              >
                <span className="inline-block">Descripción</span>
              </div>
              <div
                className={
                  "font-title " +
            ...
            {/* Añadir acciones de producto Finalizado*/}
            ...
            {/* Añadir acción fiscal finalizada*/}
...
//                    outlined={1}
                    ...
                    block={1}
                    secondary={1}
                    onClick={() => saveAs("Borrador")}
                  >
                    <CheckCircleIcon className="h-5 w-5 mr-1" /> Guardar como borrador
                  </Button>
                </div>
              )}
...
//                    outlined={1}
                    ...
                    block={1}
                    danger={1}
                    onClick={() => saveAs("Pendiente")}
                  >
                    <DollarIcon className="h-5 w-5 mr-1" />{" "}
                    {params.id === "new" ? "Guardar" : "Update"} como pendiente
                  </Button>
                </div>
              )}
              <div className="w-full flex-1 my-1 sm:my-1 md:my-0 px-1">
                <Button
                  ...
                  block={1}
                  success={1}
                  onClick={() => saveAs("Pagado")}
                >
                  <SecurityIcon className="h-5 w-5 mr-1" />{" "}
                  {params.id === "new" ? "Guardar" : "Update"} como pagado
                </Button>
              </div>
            </div>
          </div>
